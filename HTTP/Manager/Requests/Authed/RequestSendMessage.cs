...
public class RequestSendMessage
{
    public static void Process(OpenCordAddress openCordAddress, string entityBody, HttpResponse response, string ip, HttpRequest request, RegisteredUser user, WebSocketUser wsUser, ZlibStreamContext zlibContext, string[] IDs, string[] numbers)
    {
        ...
        if (!JSONUtils.IsJsonOrderValid(entityBody, new string[] { "content", "nonce", "tts" }))
        {
            ...
            if (location.channelID.Equals(channelID))
            {
                ...
                foreach (string member in members)
                {
                    if (member.Equals(user.id))
                    {
                        ...
                        exit = true;
                        message.id = Utils.GetNewID();
                        message.content = jss.content;
                        message.channel_id = channelID;
                        message.author = new MessageAuthor();
                        message.author.avatar = user.avatar;
                        message.author.discriminator = user.discriminator;
                        message.author.id = user.id;
                        message.author.public_flags = user.public_flags;
                        message.author.username = user.username;
                        message.timestamp = TimestampUtils.GetDiscordTime();
                        message.nonce = jss.nonce;
                        DataManager.SaveChannel(location.channelID, message);
                        msg = JsonConvert.SerializeObject(message);
                        Utils.InjectResponse(request, response, msg, 200, "OK");
                        break;
                    }
                }
